...
describe("today view model", () => {
  ...
  it("formats failed screenshot analysis feedback for the today page", () => {
    expect(toCaptureAnalysisWarningMessage(2, "API Key 未保存")).toBe(
      "有 2 张截图没有生成 AI 分析结果：API Key 未保存"
    );
    expect(toCaptureAnalysisWarningMessage(0, "")).toBe("");
  });
});
